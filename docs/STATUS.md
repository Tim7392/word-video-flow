...
| **P1 人工门禁：剪映草稿可编辑性 —— 人验收通过（Tim，2026-09-16）** | **验收对象**：`D:\jianying\JianyingPro Drafts\单词速记_生产251-300待验_20260916`（**真实生产批次 251-300 的 50 词草稿**，162 文件 / 437MB，13 轨 / 150 条配音）。**六步全部正常、无异常**：改一个词 → 移音标 → 换一条配音 → 换背景 → 保存重开 → 导出约 5 秒并连续播放。**环境范围**：本机 + 剪映 **11.4.2.14459**（product 11.4.2.f0a65bde0d6，H0 从 `%LOCALAPPDATA%\JianyingPro\Apps\` 探测）。**这意味着**：机器只能证明结构与时间一致，"人打开能改、改完能存能导"这一条**现在有人证**了。**仍未覆盖**：跨机、跨剪映版本、成员独立操作（另需分别复验）；不承诺永久支持任何最新版 |
