    ...
    if args.action == 'schema':
        return {'version': 1, 'actions': sorted(commands.choices),
                ...
                'notes':['Use provider local with items[index,role,text,voice,path] for original unscaled audio.',
                         'lesson.spoken_policy decides how the Chinese reading text is derived from the display meaning: v2 (default, approved cleaning) or legacy (reproduce a batch delivered before the cleaning). It cannot be combined with lesson.entries, whose reading text is authoritative as given.',
